...
ramda.js の constructN 関数は、引数の数を指定して、クラスやコンストラクタ関数を引数にとり、そのインスタンスを作成する関数を返す関数ですわ。⁴

例えば、以下のような Point クラスがあるとしますわ。
...
このクラスに constructN 関数を適用すると、以下のようになりますわ。
...
// makePoint関数は、xとyを引数にとり、Pointのインスタンスを返す関数になる
const p1 = makePoint(3, 4); // x: 3, y: 4
const p2 = makePoint(5, 12); // x: 5, y: 12

// p1とp2はPointのインスタンスなので、distanceToOriginメソッドを呼び出せる
console.log(p1.distanceToOrigin()); // 原点からの距離は5です
console.log(p2.distanceToOrigin()); // 原点からの距離は13です
```
